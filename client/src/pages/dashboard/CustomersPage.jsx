...
export default function CustomersPage() {
  ...
  const fetchCustomers = useCallback(() => {
    ...
    api.get(`/store/customers?${params.toString()}`)
      .then((res) => {
  ...
  return (
    <div className="flex min-h-screen w-full bg-bg text-text">
      ...
      <div className="flex-1 p-6 overflow-auto max-w-2xl">
        ...
        <p className="text-text-muted text-sm mb-6">Everyone with an account on your store.</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 rounded-xl border border-border bg-surface">
            <p className="text-sm text-text-muted">{query ? "No matches found." : "No customers yet."}</p>
          </div>
        ) : (
          <>
                ...
                >
                  ...
                  {confirmingId === c.id ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleRemove(c.id)} className="text-xs text-danger font-medium">Confirm</button>
                      <button onClick={() => setConfirmingId(null)} className="text-xs text-text-muted">Cancel</button>
                    </div>
                  ) : (
                    <button
            ...
            {meta && meta.totalPages > 1 && (
              <div className="flex items-center justify-between mt-4">
                <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="text-sm text-text-muted hover:text-text disabled:opacity-30">
                  Previous
                </button>
                <span className="text-xs text-text-muted">Page {meta.page} of {meta.totalPages}</span>
                <button disabled={page >= meta.totalPages} onClick={() => setPage((p) => p + 1)} className="text-sm text-text-muted hover:text-text disabled:opacity-30">
                  Next
                </button>
              </div>
            )}
